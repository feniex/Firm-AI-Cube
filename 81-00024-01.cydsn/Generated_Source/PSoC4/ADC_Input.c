...
#include "ADC_Input.h"
    ...
    #define ADC_Input_SetP4PinDriveMode(shift, mode)  \
    do { \
        ADC_Input_PC =   (ADC_Input_PC & \
                                (uint32)(~(uint32)(ADC_Input_DRIVE_MODE_IND_MASK << \
                                (ADC_Input_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (ADC_Input_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define ADC_Input_SetP4PinDriveMode(shift, mode)  \
        do { \
            ADC_Input_USBIO_CTRL_REG = (ADC_Input_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(ADC_Input_DRIVE_MODE_IND_MASK << \
                                    (ADC_Input_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (ADC_Input_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		ADC_Input_SetP4PinDriveMode(ADC_Input__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(ADC_Input_DR & (uint8)(~ADC_Input_MASK));
    drVal = (drVal | ((uint8)(value << ADC_Input_SHIFT) & ADC_Input_MASK));
    ADC_Input_DR = (uint32)drVal;
}
...
{
    return (uint8)((ADC_Input_PS & ADC_Input_MASK) >> ADC_Input_SHIFT);
}
...
{
    return (uint8)((ADC_Input_DR & ADC_Input_MASK) >> ADC_Input_SHIFT);
}
...
void ADC_Input_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  ADC_Input_INTCFG & (uint32)(~(uint32)position);
    ADC_Input_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(ADC_Input_INTSTAT & ADC_Input_MASK);
	ADC_Input_INTSTAT = maskedStatus;
    return maskedStatus >> ADC_Input_SHIFT;
}
