         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        *  UART_Out_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(UART_Out_BL_CHK_DELAY_MS);
        }
    }
    ...
    while(UART_Out_GetRxBufferSize() > 0u)
    {
        ...
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            ...
            *  If not wait for additional UART_Out_WAIT_EOP_DELAY ms.
            */
