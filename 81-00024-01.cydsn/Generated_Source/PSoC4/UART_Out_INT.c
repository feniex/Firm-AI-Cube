...
/* `#END` */
    ...
    *******************************************************************************/
    CY_ISR(UART_Out_RXISR)
    {
    ...
    #ifdef UART_Out_RXISR_ENTRY_CALLBACK
        UART_Out_RXISR_EntryCallback();
    #endif /* UART_Out_RXISR_ENTRY_CALLBACK */
        ...
        do
        {
            ...
            readStatus = UART_Out_RXSTATUS_REG;
            /* Copy the same status to readData variable for backward compatibility support 
            *  of the user code in UART_Out_RXISR_ERROR` section. 
            */
            readData = readStatus;

            if((readStatus & (UART_Out_RX_STS_BREAK | 
                            UART_Out_RX_STS_PAR_ERROR |
                            UART_Out_RX_STS_STOP_ERROR | 
                            UART_Out_RX_STS_OVERRUN)) != 0u)
            {
                /* ERROR handling. */
                UART_Out_errorStatus |= readStatus & ( UART_Out_RX_STS_BREAK | 
                                                            UART_Out_RX_STS_PAR_ERROR | 
                                                            UART_Out_RX_STS_STOP_ERROR | 
            ...
            #ifdef UART_Out_RXISR_ERROR_CALLBACK
                UART_Out_RXISR_ERROR_Callback();
            #endif /* UART_Out_RXISR_ERROR_CALLBACK */
            }
            
            if((readStatus & UART_Out_RX_STS_FIFO_NOTEMPTY) != 0u)
            {
                ...
                {
                    if((readStatus & UART_Out_RX_STS_MRKSPC) != 0u)
                    {
                        if ((readStatus & UART_Out_RX_STS_ADDR_MATCH) != 0u)
                        {
        ...
        }while((readStatus & UART_Out_RX_STS_FIFO_NOTEMPTY) != 0u);
    ...
    #ifdef UART_Out_RXISR_EXIT_CALLBACK
        UART_Out_RXISR_ExitCallback();
    #endif /* UART_Out_RXISR_EXIT_CALLBACK */
    ...
    *******************************************************************************/
    ...
    #ifdef UART_Out_TXISR_ENTRY_CALLBACK
        UART_Out_TXISR_EntryCallback();
    #endif /* UART_Out_TXISR_ENTRY_CALLBACK */
    ...
    #ifdef UART_Out_TXISR_EXIT_CALLBACK
        UART_Out_TXISR_ExitCallback();
    #endif /* UART_Out_TXISR_EXIT_CALLBACK */
