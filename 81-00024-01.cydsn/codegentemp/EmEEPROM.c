...
*******************************************************************************/
#if (CY_PSOC3)
    cystatus EmEEPROM_Write(const uint8 srcBuf[], const uint8 eepromPtr[], uint16 byteCount)
...
#endif /* (!CY_PSOC3) */
{
    ...
    #if (CY_PSOC3)
        eeOffset &= EmEEPROM_CODE_ADDR_MASK;
    #endif /* (CY_PSOC3) */

    /* Make sure, that eepromPtr[] points to ROM */
    #if (CY_PSOC3)
        if (((uint32)eepromPtr >= EmEEPROM_CODE_ADDR_OFFSET) &&
            (((uint32)eepromPtr + byteCount) <= EmEEPROM_CODE_ADDR_END))
    #else
        if (((uint32)eepromPtr + byteCount) <= EmEEPROM_FLASH_END_ADDR)
    #endif /* (CY_PSOC3) */
    {
        #if (!CY_PSOC4)
            (void)CySetTemp();
        ...
        rowId = (eeOffset / CY_FLASH_SIZEOF_ROW) % EmEEPROM_ROWS_IN_ARRAY;
        ...
        while ((srcIndex < byteCount) && (CYRET_SUCCESS == rc))
        {
            ...
            rowId++;
                ...
                if (rowId >= EmEEPROM_ROWS_IN_ARRAY)
                {
                    arrayId++;
                    rowId = 0u;
                }
